...
private String type = "Dollar";
	...
	public Dollar add(Dollar a, Dollar b) {
		Dollar x = new Dollar(((a.getUnit()+b.getUnit())+(a.getFraction()+b.getFraction())/100.0));
		return x;
	}
	...
	public Dollar subtract(Dollar a, Dollar b) {
		double one = (double)a.getUnit()+(double)a.getFraction()/100;
		double two = (double)b.getUnit()+(double)b.getFraction()/100;
		if (one>=two) {
